...
## 2025-04-15 - EXISTS Subquery vs JOIN for Aggregates in SQLite
**Learning:** For queries aggregating child table records based on a filter applied to the parent table (e.g., getting "Top 5 Products" from `invoice_items` where the parent `invoice` is 'active'), an `EXISTS` clause on the child table is significantly faster (~18% speedup observed) than using a `JOIN` with a `WHERE` filter. This is because joining the tables creates a massive intermediate result set that must then be grouped. The `EXISTS` pattern allows SQLite to iterate the child items and efficiently look up the parent via index, sidestepping the intermediate result set creation.
**Action:** When aggregating records in a child table (`invoice_items`) that just need to be filtered by a simple condition on the parent table (`invoices`), use an `EXISTS` subquery instead of an `INNER JOIN` or `LEFT JOIN` to avoid heavy intermediate result sets.
